...
cashPay.addEventListener("click", paymentCash);

// Add event listener to "buttonPay"
...
TBC.addEventListener("click", () => {
  ...
  if (recivedItem) {
    console.log("ბარათი ----   გვაქვს recivedItem (რედაქტირება)");
  ...
  //შენახვა და მერე ბარათით გადახდა
    ...
    console.log("ბარათი ----  არ გვაქვს recivedItem");
...
BOG.addEventListener("click", () => {
  // newBoxes.classList.toggle("show");
  localStorage.setItem("Bank","BOG")
  newBoxes.style.display = "none";
...
cancelBtn.addEventListener("click", () => {
  fetch("http://5.152.108.245:8088/api/v1/order/delete", {
    ...
    body: JSON.stringify({
      id: JSON.parse(localStorage.getItem("orderId")),
    }),
  })
    .then((response) => {
      if (response.ok) {
...
agreeBtn.addEventListener("click", () => {
  if (localStorage.getItem("paymentType") === "ბარათი") {
    console.log("ბარათი - აგრიზე დაჭერისას უნდა მოხდეს ბარათით გადახდა");
    // Example usage:
    AgreeButtonCard(JSON.parse(localStorage.getItem("orderId")));
  } else if (localStorage.getItem("paymentType") === "ქეში") {
    console.log("ქეში - აგრიზე დაჭერისას უნდა მოხდეს ქეშით გადახდა");
    AgreeButtonCash(JSON.parse(localStorage.getItem("orderId")));
  } else {
    console.log("არ გვაქვს გადახდის ტიპი მითითებული");
  }
});

payWithCard.addEventListener("click", () => {
  console.log("ბარათის არჩევა");
  newBoxes.style.display = "block";
  localStorage.setItem("paymentType", "ბარათი");
...
function paymentCash() {
  console.log("ქეშის არჩევა");
  localStorage.setItem("paymentType", "ქეში");

  // თუ ლოკალ სთორიჯში არის უკვე შენახული ორდერის იფორმაცია if(recivedItem)
  if (recivedItem) {
    console.log("ქეში ---- გვაქვს recivedItem = (რედაქტირება)");
    let basketContainer = document.querySelector(".right-basket-container");
    ...
    fetch("http://5.152.108.245:8088/api/v1/order/update", {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          ////////////////////////////////////// გადახდა
          // let agreeBtnCash = document.querySelector(".test");
          // agreeBtnCash.addEventListener("click",() => AgreeButtonCash(orderDetails.id))
        } else {
          throw new Error("Request failed with status " + response.status);
        }
      })
    ...
    console.log("ქეში ---- არ გვაქვს recivedItem ");
    ...
    fetch("http://5.152.108.245:8088/api/v1/order/save", {
      ...
      .then((orderId) => {
        ...
        if (orderId) {
          ...
          // let agreeBtnCash = document.querySelector(".test");
          // agreeBtnCash.addEventListener("click",() => AgreeButtonCash(orderId))
        }
...
//////////////////////pay
function AgreeButtonCard(orderId) {
  console.log("ბარათი");
  numericContainer.style.display = "none";
  ...
  // Payment was successful, call the AgreeButtonCash function
  ...
  // მეორედ შემოსვლისას 0 - numberInputValue და ვიღებთ არასწორს შედეგს
...
// განვსაზღვრავთ რომელ ბანკის ტერმინალის ენდ ფოინთზე გავგზავნით რექვესთებს
  let BankPoint = localStorage.getItem("Bank") === "TBC" ? TBCpoint : BOGpoint

  payWithTerminal(numberInputValue,BankPoint)
  .then(() => {
    if (numberInputValue > orderFullPrice.textContent) {
      alert("ბარათით ზედმეტის გადახდა არ ხდება");
    }
  ...
  .catch(() => {
    // Payment was unsuccessful, handle the error (if needed)
    console.error("Payment with TBC failed.");
  });

}

function AgreeButtonCash(orderId) {
  console.log("ქეში");
  let BasketContainer = document.querySelector(".right-basket-container");
  if (BasketContainer.innerHTML === "") {
    alert("empty");
...
function payWithTerminal(payAmount,requestPoint) {
  return new Promise((resolve, reject) => {
    openPos();

    function openPos() {
      // console.log(parseInt(orderFullPrice.innerHTML).toFixed().replace('.', '') * 100,)
      fetch(`${requestPoint}/openpos`, {
        method: "POST",
        headers: {
    ...
    function unlockDevice(accesToken) {
      fetch(`${requestPoint}/executeposcmd`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    function transactionStatus() {
      fetch(`${requestPoint}/getevent`, {
        ...
        .then((data) => {
          console.log(
            data,
            "ტრანზაქციის სტატუსის რექვესთის დროს ონდისფლაიტექსტ"
          );
    ...
    // function chechReceipt(){
    //   console.log("Semowmeba")
    ...
    //     .then((response) => response.json())
    //     .then((data) => {
    //       console.log(data,'ONDISPLAYTEXT')

    //       if(data.eventName === 'ONDISPLAYTEXT'){
    //         console.log(data)
    //         console.log("ONDISPLAYTEXT დაიჭირა ")
    //       }else{
    //         chechReceipt();
    //       }
    //       }).catch((error) => {
...
window.addEventListener("beforeunload", function (event) {
  // Remove the "orderId" from the "localStorage"
  localStorage.removeItem("orderId");
});
